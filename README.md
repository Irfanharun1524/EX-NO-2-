...
def m(k):
    ...
    k+="".join(c for c in a if c not in k)
    return[[k[i*5+j]for j in range(5)]for i in range(5)]

def f(x,c):
    for i in range(5):
        for j in range(5):
            if x[i][j]==c:return i,j
    return 0,0
...
def p(a,b,x,e):
    ...
    r2,c2=f(x,b)
    s=1 if e else 4
    if r1==r2:return x[r1][(c1+s)%5],x[r2][(c2+s)%5]
    if c1==c2:return x[(r1+s)%5][c1],x[(r2+s)%5][c2]
    return x[r1][c2],x[r2][c1]

def e(t,x,enc):
    t=''.join(c.upper().replace('J','I')for c in t if c.isalpha())
    r=""
    i=0
    while i<len(t):
